...
const psexEnums = require("./ps.enums");
const psexMethods = require("./ps.methods");
const psexProperties = require("./ps.propaties");
function isPsexScript(line) {
    return line.indexOf("#target photoshop;") != -1;
}
function getJoinString(text, separator) {
...
function activate(context) {
    const jsx_selector = { scheme: 'file', language: 'jsx' };
    // app定義
    const app_provider = vscode.languages.registerCompletionItemProvider(jsx_selector, {
        provideCompletionItems(document, position) {
            if (!isPsexScript(document.lineAt(0).text))
                return;
            let completions = [];
    ...
    psexMethods
        ...
        const methods_provider = vscode.languages.registerCompletionItemProvider(jsx_selector, {
            provideCompletionItems(document, position) {
                if (!isPsexScript(document.lineAt(0).text))
                    return;
                let completions = [];
                    ...
                    methods.forEach(e => {
                        ...
                        if (e.params != null) {
                            ...
                            for (var i = 0; i < method_params.length; i++) {
                                if (e.params[i] == undefined || e.params[i] == null)
                                    break;
    ...
    psexProperties
        ...
        const propaties_provider = vscode.languages.registerCompletionItemProvider(jsx_selector, {
            provideCompletionItems(document, position) {
                if (!isPsexScript(document.lineAt(0).text))
                    return;
                let completions = [];
                ...
                if (new RegExp(regStr).test(linePrefix)) {
                    properties.forEach(p => {
                        const completion = new vscode.CompletionItem(p.name, vscode.CompletionItemKind.Field);
    ...
    psexEnums
        ...
        const enum_provider = vscode.languages.registerCompletionItemProvider(jsx_selector, {
            provideCompletionItems(document, position) {
                if (!isPsexScript(document.lineAt(0).text))
                    return;
                let completions = [];
                ...
                if (new RegExp(`${prefix}.$`).test(linePrefix)) {
                    enums.forEach(e => {
                        const completion = new vscode.CompletionItem(e.name, vscode.CompletionItemKind.EnumMember);
